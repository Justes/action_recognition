...
from .vitmodule import Attention, PreNorm, FeedForward
import numpy as np
...
def vivit_model(num_classes, num_frames=16, pretrained=True, **kwargs):
    model = ViViT(image_size=224, patch_size=16, num_classes=num_classes, num_frames=num_frames)
    if pretrained and kwargs.get("pretrained_model") != "":
        init_pretrained_weights(model, kwargs.get("pretrained_model"))
        # init_from_vit_pretrain_(module=model,
        #                         pretrained=kwargs.get("pretrained_model"),
        #                         conv_type='Conv3d',
