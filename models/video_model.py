...
class VideoTransformer(pl.LightningModule):
	...
	def __init__(self, 
		...
		if self.configs.objective =='mim': 
			...
			self.max_top1_acc = 0
			self.train_top1_acc = Accuracy(task='multiclass', num_classes=num_classes)
			self.train_top5_acc = Accuracy(task='multiclass', num_classes=num_classes, top_k=5)
		...
		if self.do_eval:
			self.val_top1_acc = Accuracy(task='multiclass', num_classes=num_classes)
			self.val_top5_acc = Accuracy(task='multiclass', num_classes=num_classes, top_k=5)
		if self.do_test:
